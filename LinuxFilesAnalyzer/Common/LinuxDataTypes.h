...
struct LinuxLogEntry {
    ...
    int64_t unixTimestamp;      // Unix timestamp
    std::string hostname;       // Hostname
    std::string process;        // Process name
    int pid;                    // Process ID (-1 if not available)
...
struct LinuxUserInfo {
    ...
    int64_t lastPasswordChange; // Last password change (days since epoch)
    int passwordMaxAge;         // Max password age in days
    int passwordMinAge;         // Min password age in days
    int passwordWarnDays;       // Password warning days
    int inactiveDays;           // Days after password expires until account is disabled
    int64_t accountExpires;     // Account expiration date
    bool isLocked;              // Is account locked
    bool isSystemAccount;       // Is system account (uid < 1000 typically)
};
...
struct LinuxLoginRecord {
    ...
    int64_t loginTime;          // Login timestamp
    int64_t logoutTime;         // Logout timestamp (0 if still logged in)
    std::string loginType;      // Login type: login, ssh, console, reboot, shutdown
    bool isSuccess;             // Success (wtmp) or failure (btmp)
    int pid;                    // Process ID
};
...
struct ShellHistoryEntry {
    ...
    int64_t timestamp;          // Timestamp (if available, e.g., HISTTIMEFORMAT)
    int lineNumber;             // Line number in history file
...
struct SSHKnownHost {
    ...
    bool isHashed;              // Is hostname hashed
};
...
struct PackageInfo {
    ...
    int64_t installTime;        // Installation timestamp
...
struct NetworkConnection {
    ...
    int uid;                    // User ID of connection owner
    int inode;                  // Socket inode
    std::string process;        // Process name (if available)
    int pid;                    // Process ID (if available)
};
...
struct SystemdServiceInfo {
    ...
    bool isEnabled;             // Is enabled at boot
};
...
struct FirewallRule {
    ...
    int sourcePort;             // Source port (-1 for any)
    int destinationPort;        // Destination port (-1 for any)
...
struct LinuxAuditLogEntry {
    ...
    int serialNumber;           // Audit serial number
